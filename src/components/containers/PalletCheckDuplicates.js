...
function PalletCheckDuplicates({ eel, params, setParams }) {
    // console.log(params);
    ...
    const columns = [
        ...
        { title: "carton_num", field: "carton_num" },
        { title: "pallet_num", field: "pallet_num" },
        { title: "inv_num", field: "inv_num" },
    ...
    const handleBarcodeInput = (event) => {
        let pallet_num = event.target.value.trim();
        pallet_num = pallet_num ? pallet_num.toUpperCase() : ""
        const inv_num = pallet_num;
        // alert(`___${pcb_sn}___`);
        console.log(params);
        try {
            if (toggle && !pallet_num.startsWith('SKW')) {
                alert("Invalid Pallet Number");
                return;
            }
            if (!toggle && !inv_num.startsWith('DN')) {
            ...
            if (params.server.status) {
                eel.pallet_check_duplicates(
                    ...
                    try {
                        ...
                        console.log(metadata.stb_list)
                        setState({
                            ...state,
                            pallet_num: pallet_num,
                            items: metadata.stb_list,
                            passed: metadata.stb_list?.length ? false : true,
                            unique_inv: metadata.unique_inv,
                            unique_pallet: metadata.unique_pallet,
                        });
                    } catch (error) {
                        setTimeout(() => {
                ...
                <div className="flex mb-2 border-0 border-blue-700 border-double justify-between rounded-t-lg">
                    ...
                    <div className="flex border-0 border-red-500 flex-1 justify-between">
                        <div className="flex flex-col border-0 border-green-500 ">
                            <label className="text-black label">
                                <span className="text-black label-text">
                                    {toggle ? 'Scan Pallet No.' : 'Enter Invoice No.'}
                                </span>
                            </label>
                            <input
                                type="text"
                                placeholder={toggle ? 'Pallet No.' : 'Invoice No.'}
                        ...
                        <div className="my-auto">
                            {state.passed ? (
                                <div className="text-green-600 md:text-xl lg:text-2xl">
                                    {state.pallet_num} has all new production
                                    units ✔
                                </div>
                            ) : (
                                <div className="text-red-600 md:text-xl lg:text-2xl">
                                    {state.pallet_num} contains previously dispatched
                                    units. ❌
                                </div>
                            )}
                        </div>
                        ...
                        <div className="stats stats-vertical sm:stats-horizontal">
                            <div className="stat">
                                ...
                                <div className="stat-value">{state.unique_inv}</div>
                            </div>
                            <div className="stat">
                                <div className="stat-title">Pallets</div>
                                <div className="stat-value">{state.unique_pallet}</div>
                            </div>
                            {/* <div className="stat">
...
export default PalletCheckDuplicates;
